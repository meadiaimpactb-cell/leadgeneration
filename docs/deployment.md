...
> **للنشر المتكرّر: `./deploy.sh`** — وهو هذا الترتيب نفسه منفَّذًا، مع وضع
> الموقع في وضع الصيانة أثناء النافذة التي تختلف فيها قاعدة البيانات عن
> الأصول، وإعادة تشغيل SSR، وفحصٍ بعديّ يرفض النجاح ما لم تكن `/ar` مرسومة
> من الخادم فعلًا. آمن لإعادة التشغيل.
>
> الخطوات أدناه هي المرجع، وتلزم في **أول** نشر (المفاتيح، `storage:link`،
> `amad:create-admin`، supervisor). ما بعد ذلك يفعله السكربت.
...
# إن فشل بـ symlink(): disabled — كما على Hostinger — فلا تعدّل
# config/filesystems.php على الخادم. اضبط بدلًا منها في .env:
#   MEDIA_DISK_ROOT=/المسار/الكامل/إلى/public/storage
# ثم أنشئ المجلد وامنحه صلاحية الكتابة. راجع «تخزين الوسائط» أدناه.
...
SESSION_SECURE_COOKIE=true      # الموقع على https — بدونها تُرسَل الجلسة على http أيضًا
...
> ⚠️ **لا تضع نفس الوسم في المكانين.** حقول GA4 وMeta Pixel وClarity في اللوحة
> تُصدر الوسم بنفسها. فإن أضفتم نفس الوسم **أيضًا** كوسم داخل Google Tag Manager،
> أُطلق مرتين وتضاعفت كل زيارة وكل تحويل في التقارير. اختاروا أحد الطريقين:
> إمّا تملأون الحقول الثلاثة في اللوحة وتتركونها خارج GTM، أو تُديرونها كلها من
> GTM وتتركون الحقول الثلاثة **فارغة** وتملأون حقل GTM وحده.
>
> `tracking.meta_capi_token` استثناء: تكامل Conversions API لم يُبنَ بعد، والحقل
> مُهيَّأ لاستقباله فقط — قيمته لا تُصدَر إلى الصفحة ولا تُرسِل شيئًا اليوم، وهذا
> ما تقوله الشاشة نفسها.

### تخزين الوسائط على استضافة بلا symlink

عرف Laravel أن الوسائط تُكتب في `storage/app/public` وتُقرأ عبر وصلة رمزية من
`public/storage`. هذا صحيح في كل مكان يمكن فيه إنشاء الوصلة — ولا يمكن على
الاستضافة الحالية، إذ `symlink()` مُدرجة في `disable_functions`. وبلا وصلة
وبلا بديل تُعيد **كل** صورة 404: الموقع يعمل وصوره كلها مفقودة.

لذلك جذر القرص `public` متغيّر بيئة لا ثابت في الكود:

```env
# على الخادم فقط. المسار الكامل للمجلد الذي يخدمه الويب مباشرة.
MEDIA_DISK_ROOT=/home/u153052345/domains/amadcraft.sa/public_html/b2b/public/storage
```

```bash
mkdir -p "$MEDIA_DISK_ROOT" && chmod 755 "$MEDIA_DISK_ROOT"
```

**لا تعدّل `config/filesystems.php` على الخادم.** كان الخادم يحمل تعديلًا يدويًا
هناك، ومعناه أن أول نشر يلمس `config/` يعيد أعطال الـ404 صامتة. القيمة الفارغة
تُعامَل كغير مضبوطة، فيعود الافتراضي وتعمل `storage:link` كالمعتاد محليًا.

> الصور الحالية جاهزة في `media-storage.zip` — تُرفع إلى المسار أعلاه وتُفكّ فيه
> مباشرة، لا في مجلد فرعي: مسارات `media` في قاعدة البيانات نسبية إليه.
...
# جدولة Laravel نفسها. سطر واحد يشغّل كل ما هو مجدول داخل التطبيق —
# اليوم: الملخص اليومي للعملاء المحتملين (السابعة صباحًا، من شاشة التنبيهات).
# بدون هذا السطر لا يُرسَل الملخص أبدًا، **ولا يظهر أي خطأ**: المهمة لا تفشل،
# هي ببساطة لا تُستدعى. وهذا أسوأ من الفشل لأنه يبدو كيوم بلا طلبات.
* * * * * cd /var/www/amadcraft && php artisan schedule:run >> /dev/null 2>&1
```

> **وتذكير مرتبط:** الملخص وتنبيه الطلب الجديد كلاهما يمرّ بالطابور (§6.2)،
> فبلا `queue:work` قيد التشغيل لا يصل بريد أصلًا — راجع «العمليتان الدائمتان»
> في `docs/final-review.md`.
